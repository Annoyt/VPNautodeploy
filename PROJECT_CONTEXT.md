...
### Entry node SSH hardening — 2026-08-05

Вход по паролю на entry-узле **отключён**. До этого он был открыт: при
`PermitRootLogin yes`, `PasswordAuthentication yes` и заданном пароле root, без
fail2ban, на узел приходило несколько тысяч попыток подбора в сутки (пиковые
адреса — по 1374 попытки от каждого). Успешных входов не было ни одного, кроме
своих, но запас прочности держался только на стойкости пароля.

Что сделано — `/etc/ssh/sshd_config.d/01-harden.conf`:
...
Откат: удалить файл и `systemctl reload ssh`. Резервная копия основного
конфига — `/etc/ssh/sshd_config.bak.<дата>`.

> **Почему имя начинается с 01, а не с 99.** В sshd побеждает **первое**
> вхождение параметра, а `50-cloud-init.conf` ставит `PasswordAuthentication
> yes`. Файл с бо́льшим номером читается позже и силы не имеет — первая попытка
> с именем `99-harden.conf` применила только `PermitRootLogin`, а запрет
> паролей молча не сработал. Если cloud-init когда-нибудь перепишет свой файл,
> `01-harden.conf` всё равно останется первым.

### Entry node как транзит к промышленному оборудованию — 2026-08-05

На entry-узле заведён непривилегированный пользователь `tunnel` и поднят
обратный SSH-туннель с объекта, находящегося за NAT. Через него доступен шлюз
управления кавитатором (проект `qavitator`), а с него — контроллер, коммутирующий
силовой контактор.

| | |
|---|---|
| Пользователь | `tunnel`, оболочка `nologin`, только проброс порта |
| Порт | `2222`, слушает **только `127.0.0.1`** (`GatewayPorts` оставлен `no`) |
| Ключ | создан на самом шлюзе, узел его не хранит |
| Ограничение ключа | `restrict,port-forwarding,command="/bin/false"` |

**Это меняет цену компрометации узла.** Раньше на кону был VPN-сервис, теперь
ещё и доступ к промышленной установке. Отсюда и запрет паролей выше — он
перестал быть гигиеной и стал необходимостью.

При переустановке узла воссоздать: пользователя `tunnel`, его
`authorized_keys` и запрет паролей. Скрипты развёртывания VPN этого не делают —
они вообще не трогают ни sshd, ни пользователей, ни iptables целиком, так что
переустановка туннель не сломает, но и не восстановит.
